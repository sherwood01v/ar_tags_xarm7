...
def callback(data):
    if len(data.markers)!=0:
        ...
        for marker_old in data.markers:
            marker = marker_old
            marker_old.pose.header.frame_id = "ar_marker_" + str(marker_old.id)
            marker_new_pose = listener.transformPose("usb_cam", marker_old.pose)
            #marker = ar_track_alvar_msgs.msg.AlvarMarker()
            marker.pose = marker_new_pose
